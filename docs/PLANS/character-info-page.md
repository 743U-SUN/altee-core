...
model CharacterInfo {
  ...
  // === 活動情報 ===
  activePlatforms      String[]  // 活動プラットフォーム: ["YouTube", "Twitch", ...]
...
**String[] フィールド**: PostgreSQL の `text[]` にマッピングされ、Prisma の `has` / `hasSome` / `hasEvery` で検索可能。
...
| フィールド | UI | 備考 |
|-----------|-----|------|
| 活動プラットフォーム | Switch付きリスト | YouTube, Twitch, ニコニコ, TwiCas, SHOWROOM, IRIAM, REALITY, 17LIVE, ミルダム, Kick, その他 |
...
各プラットフォーム名と ON/OFF スイッチ。ON にすると `activePlatforms[]` に追加。
...
```typescript
// 取得
getCharacterInfo(): Promise<CharacterInfo | null>
...
### Phase 1: DB + 基盤
1. Prisma スキーマに `CharacterInfo` モデルを追加（活動情報フィールド含む）
2. マイグレーション実行
...
| ファイル | 変更内容 |
|---------|---------|
| `prisma/schema.prisma` | CharacterInfo モデル追加、User リレーション追加 |
...
4. 未入力状態でも保存・ページ遷移がエラーにならないこと
5. `npm run lint && npx tsc --noEmit` がエラーゼロであること
