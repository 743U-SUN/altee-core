...
- [ ] **Session 10** — `components/user-profile/` ルート + `header-edit/`（~17 files）
...
- [ ] **Session 11** — `components/user-profile/sections/`（レンダラー、26 files）
...
- [ ] **Session 12** — `components/user-profile/sections/editors/`（23 files）
...
- [ ] **Session 13** — `components/layout/` + `components/sortable-list/` + `components/sidebar-content/` + `components/theme-provider/`（~22 files）
...
- [ ] **Session 14** — `components/` 残り（profile, decorations, editor, image-uploader, notification, navigation, pwa, items, sections 等）（~20 files）
...
- [ ] **Session 23** — `types/` + `hooks/` + `constants/` + `prisma/schema.prisma`（~20 files）
...
4. **レポート命名規則**: `docs/REFACTOR/[target-name]-[YYYY-MM-DD].md`
5. **各セッションの実行方法**: `/nextjs-multi-agent-review [対象ディレクトリ]` で実行
