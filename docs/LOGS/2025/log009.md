...
### Phase 1: 基盤整備
- [x] UserRole enum に GUEST 追加
- [x] Prismaマイグレーション作成・適用
- [x] `/admin/users` ページ作成（空の状態）
- [x] 基本レイアウト・ナビゲーション
- [x] Server Actions ファイル作成
...
- [x] ロール変更機能
- [x] アカウント有効/無効切り替え
- [x] ユーザー強制削除（ブラックリスト追加オプション付き）
- [x] 強制ログアウト機能
- [x] ブラックリスト管理ページ
- [x] ブラックリスト CRUD操作

### Phase 4: 高度機能
- [x] 統計ダッシュボード（管理画面サイドバー）
- [x] 一括操作機能（複数ユーザー選択、一括ロール変更、一括状態変更）
- [x] CSVエクスポート ← **次に実装する機能**
- [x] エラーハンドリング改善
- [x] 操作ログ機能
...
### ✅ 完了した主要機能
1. **基盤・コア機能**: ユーザー管理システムの基本機能はすべて完成
2. **管理機能**: ロール変更、削除、ログアウト、ブラックリスト管理が完成
3. **統計ダッシュボード**: サイドバーに実際のデータを表示する統計情報を実装
4. **一括操作**: 複数ユーザーの同時操作（ロール変更、状態変更）が完成
5. **ブラックリスト連携**: ユーザー削除時にブラックリスト追加オプションを実装

### 🎯 次に実装すべき機能: CSVエクスポート

**実装内容**:
- ユーザー一覧のCSVダウンロード機能
- 現在のフィルタ条件を反映したエクスポート
- ユーザー基本情報（名前、メール、ロール、状態、登録日）を含む
- `/admin/users` ページにエクスポートボタンを追加

**技術要件**:
- Server Action でCSVデータ生成
- フィルタ条件をServer Actionに渡す
- ブラウザでCSVファイルダウンロード
- shadcn/ui Button コンポーネント使用
