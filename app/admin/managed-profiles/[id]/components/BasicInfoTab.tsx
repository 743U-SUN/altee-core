...
export function BasicInfoTab({ userId, initialData }: BasicInfoTabProps) {
  ...
  const onSubmit = async (values: BasicInfoInput) => {
    try {
      const result = await adminUpdateCharacterInfo(userId, {
        ...values,
      ...
      if (result.success) {
        toast.success("キャラクター情報を保存しました")
      } else {
        toast.error(result.error || "保存に失敗しました")
      }
    } catch {
      toast.error("キャラクター情報の保存に失敗しました")
    }
  }
  ...
  return (
    <FormProvider {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <Card>
          <CardHeader>
            <CardTitle>キャラクター基本情報</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <Button type="submit" disabled={form.formState.isSubmitting}>
              {form.formState.isSubmitting ? "保存中..." : "キャラクター情報を保存"}
            </Button>
          </CardContent>
        </Card>
      </form>
