...
export async function getYouTubeMetadata(url: string) {
  try {
    ...
    if (!/^[\w-]{11}$/.test(videoId)) {
      return { success: false, error: "有効なYouTube URLまたはVideo IDではありません" }
    }

    // YouTube oEmbed APIから動画メタデータを取得
    const metadataResult = await fetchYoutubeVideoMetadata(videoId)

    if (!metadataResult.success) {
      return { success: false, error: metadataResult.error || "動画情報の取得に失敗しました" }
    }
